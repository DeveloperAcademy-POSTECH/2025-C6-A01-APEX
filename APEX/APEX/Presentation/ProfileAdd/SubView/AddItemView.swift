    ...
    private var content: some View {
        List {
            ...
            toggleRow(title: "회사 업종", isOn: $draft.showsIndustry)
                .listRowSeparator(.hidden)
            toggleRow(title: "주소", isOn: $draft.showsAddress)
                .listRowSeparator(.hidden)
            toggleRow(title: "팩스번호", isOn: $draft.showsFax)
                .listRowSeparator(.hidden)
            toggleRow(title: "연매출", isOn: $draft.showsRevenue)
                .listRowSeparator(.hidden)
            toggleRow(title: "근무 인원", isOn: $draft.showsEmployees)
                .listRowSeparator(.hidden)
        }
        .listStyle(.plain)
